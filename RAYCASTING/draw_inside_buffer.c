...
int	get_pixel_color(t_texture *texture, int texture_x, double tex_pos)
{
	int			texture_y;
	int			offset;
...
void	draw_wall(t_data *data, int window_x)
{
	...
	texel_step = (double)texture->height / data->wall.height;
	texel_y_position = (data->wall.start - WIN_HEIGHT / 2 + data->wall.height / 2) * texel_step;

	window_y = data->wall.start;
	while (window_y < data->wall.end)
	{
		buffer_pixel_put(data, window_x, window_y, rgb(
			get_pixel_color(texture, texture_x, texel_y_position), NULL));
		texel_y_position += texel_step;
		window_y++;
	}
}
